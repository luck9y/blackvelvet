    ...
    ? errors.map(error => `
        <article class="log-card console-error-card">
          ...
          <div class="log-grid">
            ...
            <div class="log-field"><span>Time</span><strong>${escapeHtml(error.created_at)}</strong></div>
          </div>
...
clearButton?.addEventListener("click", async () => {
  ...
  const { error } = await supabase.from("system_errors").delete().gte("id", 0);
...
if (portal) {
  new MutationObserver(setConsoleVisibility).observe(portal, {
    attributes: true,
